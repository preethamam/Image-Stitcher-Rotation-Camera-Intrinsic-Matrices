function stitched = stitchImages_largeROI(images, cameras, blendFeather, ref_idx)
    % STITCHIMAGES_LARGEROI Stitch multiple images into a panorama.
    %   stitched = STITCHIMAGES_LARGEROI(images, cameras, ref_idx, blendFeather)
    %   stitches the input images into a single panorama using the provided 
    %   camera parameters and reference image index with an automatic conservative 
    %   (larger) bounds greater than required homographies.
    %
    %   Slower than the STITCHIMAGES_TIGHTROI ~ 2.1066 seconds for the given
    %   images
    %
    %   Inputs:
    ...
    % Set output size to 3x the reference image
    output_w = w * 3;
    output_h = h * 3;
    output_view = [output_h, output_w];

    % Calculate center offset for reference image
    ref_offset_x = (output_w - w) / 2;
    ref_offset_y = (output_h - h) / 2;

    % Create translation matrix to center reference image
    T = [1 0 ref_offset_x;
         0 1 ref_offset_y;
         0 0 1];    
    ...
    for i = 1:length(images)
        if i == ref_idx
            ...
            warped = zeros(output_h, output_w, 3, 'uint8');
            warped(round(ref_offset_y+1):round(ref_offset_y+h), ...
                  round(ref_offset_x+1):round(ref_offset_x+w), :) = images{i};
            mask = zeros(output_h, output_w);
            mask(round(ref_offset_y+1):round(ref_offset_y+h), ...
                round(ref_offset_x+1):round(ref_offset_x+w)) = 1;
        else
            % Compute homography from reference to current view (changed order)
